...
def test_load_key_from_file_password_required(mock_rsa_key, ssh_connection):
    ...
    # Setup mock to raise password required exception
    mock_rsa_key.from_private_key_file.side_effect = paramiko.ssh_exception.PasswordRequiredException()
...
def test_load_key_from_file_with_password(mock_rsa_key, ssh_connection):
    ...
    # Verify
    assert key == mock_key
    mock_rsa_key.from_private_key_file.assert_called_with(
        "/path/to/key", password="keypass"
    ) 