...
from coinbase_agentkit.action_providers.hyperboliclabs.marketplace.utils import SSHManager
...
def test_remote_shell_success(provider, mock_rented_instances_response):
    ...
    with patch(
        "coinbase_agentkit.action_providers.hyperboliclabs.marketplace.action_provider.ssh_manager",
        mock_ssh,
    ):
...
def test_remote_shell_not_connected(provider, mock_rented_instances_response):
    ...
    with patch.object(
        provider.marketplace, "get_rented_instances", return_value=mock_rented_instances_response
    ):
...
def test_remote_shell_not_connected_no_instances(provider):
    ...
    with patch.object(
        ...
        return_value=RentedInstancesResponse(instances=[]),
    ):
        result = provider.remote_shell({"command": "ls"})
        assert (
            result
